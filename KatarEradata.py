...
# Используйте переменные окружения для имени пользователя и пароля почты.
# Ключи конфигурации должны быть 'MAIL_USERNAME' и 'MAIL_PASSWORD'.
app.config['MAIL_USERNAME'] = os.environ.get('KATARERA_MAIL_USER')
app.config['MAIL_PASSWORD'] = os.environ.get('KATARERA_MAIL_PASS')
# Убедитесь, что KATARERA_MAIL_DEFAULT_SENDER_EMAIL установлена, или используйте KATARERA_MAIL_USER как email отправителя по умолчанию.
# Важно, чтобы здесь было действительное значение email.
# Используйте KATARERA_MAIL_USER в качестве запасного варианта, если KATARERA_MAIL_DEFAULT_SENDER_EMAIL не установлена
# Первый аргумент os.environ.get() должен быть именем переменной окружения.
default_sender_email = os.environ.get('KATARERA_MAIL_DEFAULT_SENDER_EMAIL', os.environ.get('KATARERA_MAIL_USER'))
app.config['MAIL_DEFAULT_SENDER'] = ('KatarEra Admin', default_sender_email)
...
1. Термины
Исполнитель – артист - исполнитель или коллектив артистов - исполнителей (музыкантов).
Произведение – законченная музыкальная форма или оригинальный текст стихов, или и то и другое вместе, созданные творческим трудом автора.
Исполнение – исполнение Произведения Исполнителем.
Фонограмма – звуковая запись исполнения Произведения Исполнителем.
Объекты – совместно Произведения, Исполнения и Фонограммы.
Альбом – Объекты, расположенные в определенной последовательности и объединенные под общим названием.
Оформление – художественное оформление Альбомов и иное, имеющее отношение к Объектам.
Объекты и Оформление, права на использование которые Правообладатель передает Правопреемнику, а также названия и состав Альбомов будут указаны в Приложениях к Договору.
Контент – данные в цифровых форматах mp3, mp4, wav, midi и прочих.
Территория – весь мир.
Срок – бессрочный.
2. Предмет Договора
Правообладатель, на основании исключительной лицензии, передает Правопреемнику на Срок и Территорию имущественные авторские права на Произведения, смежные права Исполнителя на Исполнения Произведений, смежные права изготовителя фонограммы на Фонограммы:
перерабатывать в форму Контента;
доводить до всеобщего сведения таким образом, что любое лицо может получить доступ к Объектам из любого места и в любое время по собственному выбору;
воспроизводить в целях доведения до всеобщего сведения;
использовать анонимно (без указания авторов Произведений, Исполнителей, Правообладателя);
использовать фрагментарно.
3. Права, обязанности и гарантии Сторон
Правопреемник обязуется:
использовать права, полученные по Договору, в строгом соответствии с его условиями.
Правопреемник вправе:
Использовать Оформление, изображения и биографические материалы, касающиеся Исполнителя, в целях популяризации Объектов;
получать в полном объеме вознаграждение за использование Объектов, при условии последующей выплаты доли Правообладателю в соответствии со Статьей 4 Договора;
передавать полученные по Договору права третьим лицам.
Правообладатель гарантирует, что ему принадлежат права на Объекты в необходимом для исполнения Договора объеме.
4. Конфиденциальность
Стороны признают конфиденциальной любую информацию, касающуюся заключения и содержания Договора и обязуются не разглашать ее третьим лицам без предварительного письменного на то согласия другой Стороны, за исключением случаев, когда это необходимо для раскрытия соответствующим государственным органам в случаях, определенных законодательством РФ.
5. Порядок разрешения споров
Все споры, разногласия и иные вопросы, которые возникают или могут возникнуть между Сторонами, заключившими Договор, или же их уполномоченными представителями, касающиеся выполнения Сторонами Договора, Стороны намерены решать путем переговоров.
Если Стороны не пришли к соглашению путем переговоров, то все возникшие споры должны рассматриваться судом, расположенным по месту нахождения Правопреемника.
6. Ответственность сторон
В случае, если одна из Сторон нарушит свои обязательства и гарантии по Договору, другая Сторона вправе требовать возмещения причиненного ей ущерба в полном объеме.
Уплата штрафных санкций не освобождает Стороны от выполнения предусмотренных Договором обязательств.
"""
        confirm_url = url_for('confirm_email', token=token, _external=True)
        html_body = f"<p>Добро пожаловать! Спасибо за регистрацию. Пожалуйста, подтвердите вашу почту, перейдя по ссылке: <a href='{confirm_url}'>{confirm_url}</a></p><br><pre>{license_agreement_text}</pre>"
...
# Маршрут для обновления профиля пользователя (bio, avatar)
...
def update_user_profile():
    ...
    try:
        # Получаем текущий путь к аватару, чтобы удалить старый файл
        ...
        if profile_picture_file:
            filename = secure_filename(f"user_{user_id}_avatar_{profile_picture_file.filename}")
            new_picture_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
            profile_picture_file.save(new_picture_path)
            
            updates.append("profile_picture = ?")
            params.append(new_picture_path)
        ...
        # Если был загружен новый аватар и был старый, удаляем старый файл
        if new_picture_path and current_user['profile_picture'] and os.path.exists(current_user['profile_picture']):
            if UPLOAD_FOLDER in current_user['profile_picture']:
                 os.remove(current_user['profile_picture'])

    except sqlite3.Error as e:
        conn.rollback()
        app.logger.error(f"Ошибка при обновлении профиля: {e}")
        abort(500, "Database error during profile update.")
    finally:
        if conn:
            conn.close()
...
использовать анонимно (без указания авторов Произведений, Исполнителей, Правообладателя);
использовать фрагментарно.
""" # Сокращенный текст для примера, полный текст будет слишком длинным для этого блока

    confirm_url = url_for('confirm_email', token=token, _external=True)
    html_body = f"<p>Пожалуйста, подтвердите вашу почту, перейдя по ссылке: <a href='{confirm_url}'>{confirm_url}</a></p><br><pre>{license_agreement_text}</pre>"
...
# Маршрут для увеличения счетчика прослушиваний трека
...
def increment_play_count(track_id):
    ...
    try:
        ...
        conn.rollback()
        app.logger.error(f"Ошибка при увеличении счетчика прослушиваний для track_id {track_id}: {e}")
        abort(500, "Database error during play count increment")
    finally:
        if conn:
            conn.close()
...
def block_user(user_id):
    ...
    try:
        ...
        user_info = conn.execute('SELECT email, name FROM users WHERE id = ?', (user_id,)).fetchone()
        if not user_info:
            abort(404, "User not found.")
        ...
        user_name = user_info['name'] if user_info['name'] else "Пользователь"
        subject = "Уведомление о блокировке аккаунта на KatarEra"
        html_body = f"<p>Уважаемый(ая) {user_name},</p><p>Сообщаем, что ваш аккаунт на платформе KatarEra был заблокирован.</p><p>Если у вас есть вопросы, пожалуйста, свяжитесь с администрацией.</p>"
        text_body = f"Уважаемый(ая) {user_name},\n\nСообщаем, что ваш аккаунт на платформе KatarEra был заблокирован.\n\nЕсли у вас есть вопросы, пожалуйста, свяжитесь с администрацией."
        send_email(user_email, subject, html_body, text_body)

    except sqlite3.Error as e:
        conn.rollback()
        app.logger.error(f"Ошибка при блокировке пользователя {user_id}: {e}")
        abort(500, "Database error during user blocking.")
    finally:
        if conn:
            conn.close()
    return '', 200
...
def delete_track(track_id):
    ...
    try:
        ...
        track_info = conn.execute('SELECT file_path, artwork_path FROM tracks WHERE id = ?', (track_id,)).fetchone()
        if track_info:
            # Удаляем основной файл трека
            if track_info['file_path'] and os.path.exists(track_info['file_path']):
                os.remove(track_info['file_path'])
            # Удаляем обложку трека, если есть
            if track_info['artwork_path'] and os.path.exists(track_info['artwork_path']):
                os.remove(track_info['artwork_path'])
